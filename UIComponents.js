...
import { View, Text, Pressable, TextInput, StyleSheet, Animated, ActivityIndicator } from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { Colors, Spacing, Radius, Typography, Shadows } from './theme';

export const Card = ({ children, style, onPress }) => {
  const scale = useRef(new Animated.Value(1)).current;
  const onIn  = () => Animated.spring(scale, { toValue: 0.975, useNativeDriver: true, speed: 30 }).start();
  const onOut = () => Animated.spring(scale, { toValue: 1,     useNativeDriver: true, speed: 20 }).start();
  const card  = <Animated.View style={[styles.card, { transform: [{ scale }] }, style]}>{children}</Animated.View>;
  return onPress ? <Pressable onPress={onPress} onPressIn={onIn} onPressOut={onOut}>{card}</Pressable> : card;
};
...
export const SectionHeader = ({ title, right, rightColor = Colors.blue, onRight }) => (
  <View style={styles.sectionHeader}>
    ...
    {right && <Pressable onPress={onRight}><Text style={[styles.sectionRight, { color: rightColor }]}>{right}</Text></Pressable>}
  </View>
);
...
export const PrimaryButton = ({ label, onPress, disabled, loading, style }) => {
  ...
  return (
    <Pressable onPress={onPress} onPressIn={onIn} onPressOut={onOut} disabled={disabled || loading}>
      <Animated.View style={{ transform: [{ scale }] }}>
        <LinearGradient colors={disabled ? [Colors.layer2, Colors.layer2] : ['#1D4ED8', Colors.blue]} style={[styles.primaryBtn, style]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
          {loading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={[styles.primaryBtnText, { color: disabled ? Colors.t3 : '#fff' }]}>{label}</Text>}
        </LinearGradient>
      </Animated.View>
    </Pressable>
  );
...
export const InputField = ({ label, value, onChangeText, placeholder, keyboardType = 'default', prefix, style }) => (
  <View style={[styles.inputWrap, style]}>
    ...
    <View style={styles.inputRow}>
      ...
      <TextInput value={String(value ?? '')} onChangeText={onChangeText} placeholder={placeholder} placeholderTextColor={Colors.t3} keyboardType={keyboardType} style={styles.inputText} />
    </View>
  </View>
);
...
export const Toggle = ({ value, onChange }) => {
  ...
  React.useEffect(() => { Animated.spring(anim, { toValue: value ? 1 : 0, useNativeDriver: false, speed: 30 }).start(); }, [value]);
...
export const EmptyState = ({ icon, title, sub, cta, onCta }) => (
  <View style={styles.empty}>
    ...
    {cta && <PrimaryButton label={cta} onPress={onCta} style={{ marginTop: Spacing.md }} />}
  </View>
);
...
const styles = StyleSheet.create({
  card:          { backgroundColor: Colors.layer1, borderRadius: Radius.xl, padding: Spacing.md, borderWidth: 1, borderColor: Colors.border, ...Shadows.md },
  chip:          { flexDirection: 'row', alignItems: 'center', borderRadius: 99, gap: 5 },
  ...
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Spacing.sm + 6 },
  sectionTitle:  { fontFamily: 'Syne_600SemiBold', fontSize: 14, color: Colors.t1 },
  sectionRight:  { fontSize: 12, fontWeight: '600' },
  primaryBtn:    { borderRadius: Radius.lg, paddingVertical: Spacing.md, paddingHorizontal: Spacing.lg, alignItems: 'center', justifyContent: 'center', ...Shadows.blue },
  primaryBtnText:{ fontSize: 16, fontWeight: '700', letterSpacing: 0.2 },
  inputWrap:     { backgroundColor: Colors.layer2, borderRadius: Radius.md, paddingHorizontal: Spacing.md, paddingVertical: Spacing.sm + 4, borderWidth: 1, borderColor: Colors.border, marginBottom: Spacing.sm + 2 },
  inputLabel:    { fontSize: 11, fontWeight: '600', color: Colors.t3, letterSpacing: 0.5, marginBottom: 6, textTransform: 'uppercase' },
  inputRow:      { flexDirection: 'row', alignItems: 'center' },
  inputPrefix:   { fontSize: 22, fontWeight: '700', color: Colors.t3, marginRight: 6 },
  inputText:     { flex: 1, fontSize: 22, fontWeight: '700', color: Colors.t1 },
  ...
  emptyTitle:    { fontFamily: 'Syne_700Bold', fontSize: 17, color: Colors.t1, marginBottom: 6, textAlign: 'center' },
  emptySub:      { fontSize: 13, color: Colors.t2, textAlign: 'center', lineHeight: 20 },
  skeleton:      { backgroundColor: Colors.layer2, opacity: 0.7 },
  ...
  statLabel:     { fontSize: 13, color: Colors.t2 },
  statValue:     { fontSize: 14, fontWeight: '700', color: Colors.t1 },
});
