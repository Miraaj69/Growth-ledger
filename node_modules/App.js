...
import { triggerSmartNotifications, setupNotificationHandler } from './notifications';
...
function NotifTrigger() {
  ...
  useEffect(() => {
    ...
    if (!IS_REAL_BUILD) return; // skip in Expo Go — no import, no crash
    sent.current = true;
    setupNotificationHandler();  // safe: uses require() internally
    const t = setTimeout(() => {
...
function DailyLoginTrigger() {
  const { dispatch, hydrated } = useApp();
  const sent = React.useRef(false);
  React.useEffect(() => {
    if (!hydrated || sent.current) return;
    sent.current = true;
...
function AppShell() {
  ...
  const handleThemeChange = useCallback((themeMode) => {
    set({ themeMode });
  }, [set]);

  return (
    <ThemeProvider storedMode={state.themeMode} onThemeChange={handleThemeChange}>
      <ThemedApp />
    </ThemeProvider>
  );
}
