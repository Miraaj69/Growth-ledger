// UI.js — All reusable components. Zero hardcoded colors.
...
const usePress = (scaleVal = 0.974) => {
  ...
  const onIn  = useCallback(() =>
    Animated.spring(scale, { toValue:scaleVal, useNativeDriver:true, speed:35, bounciness:1 }).start(), []);
...
export const Card = memo(({ children, style, onPress, glow }) => {
  ...
  const inner = (
    <Animated.View style={[{
      ...
      padding: SP.md,
      borderWidth: 1,
      borderColor: glow ? T.blue + '44' : T.border,
      ...SDW.card,
  ...
  return onPress ? (
    <Pressable onPress={() => { tap(); onPress(); }} onPressIn={onIn} onPressOut={onOut}>
      {inner}
    </Pressable>
  ) : inner;
});
...
export const Bar = memo(({ value = 0, total = 0, color = '#4F8CFF', h = 5 }) => {
  ...
  React.useEffect(() => {
    Animated.timing(w, { toValue: fill, duration: 800, useNativeDriver: false }).start();
  }, [fill]);
  ...
  return (
    <View style={{ backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 99, height: h, overflow: 'hidden' }}>
      <Animated.View style={{
        width, height: '100%', borderRadius: 99,
        backgroundColor: color,
        shadowColor: color, shadowOpacity: 0.4, shadowRadius: 6, elevation: 2,
      }} />
    </View>
  );
});
...
// ── SECTION HEADER ────────────────────────────────────────
// Named SecHeader to avoid ANY clash with imported SDW
...
// Alias — keep SH as exported name for screens that import it
export const SH = SecHeader;
...
export const Btn = memo(({ label, onPress, disabled, loading, color, style }) => {
  ...
  return (
    <Pressable
      onPress={() => { if (!disabled && !loading) { tap(); onPress?.(); } }}
      onPressIn={onIn}
      onPressOut={onOut}
...
export const Input = memo(({ label, value, onChange, placeholder, type = 'default', prefix, suffix, error, style }) => {
  ...
  return (
    <View style={[{
      ...
      {label && (
        <Text style={{ fontSize: 11, fontWeight: '600', color: T.t3, letterSpacing: 0.5, marginBottom: 6, textTransform: 'uppercase' }}>
          {label}
        </Text>
      )}
...
export const MonthPicker = memo(({ month, year, onChange }) => {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
      <Pressable onPress={prev} style={{ width: 30, height: 30, borderRadius: RD.sm, backgroundColor: T.l2, alignItems: 'center', justifyContent: 'center' }}>
      ...
      <Pressable onPress={next} style={{ width: 30, height: 30, borderRadius: RD.sm, backgroundColor: T.l2, alignItems: 'center', justifyContent: 'center', opacity: isCur ? 0.35 : 1 }}>
        <Text style={{ fontSize: 18, color: T.t2, lineHeight: 24 }}>›</Text>
      </Pressable>
    </View>
  );
    ...
    <View style={{ position: 'absolute', bottom: SP.lg + 4, right: SP.md, alignItems: 'flex-end', zIndex: 200 }}>
      {open && (
        <Animated.View style={{ marginBottom: SP.sm, gap: SP.sm, alignItems: 'flex-end', opacity: opac, transform: [{ translateY: tY }] }}>
          {[...actions].reverse().map((a, i) => (
            <Pressable key={i} onPress={() => { a.action?.(); setOpen(false); }} style={{
      ...
      <Pressable onPress={toggle}>
        <LinearGradient
          ...
          style={{ width: 54, height: 54, borderRadius: RD.lg + 2, alignItems: 'center', justifyContent: 'center', ...SDW.blue }}
