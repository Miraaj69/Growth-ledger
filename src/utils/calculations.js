...
export const cagrCalc = (present, future, years) =>
  years > 0
    ? ((Math.pow(future / Math.max(present, 1), 1 / years) - 1) * 100).toFixed(1)
    : 0;
...
export const emergencyTarget = (monthlyExpenses, months = 6) =>
...
export const calcScore = (s) => {
  const present   = s.attendance ? s.attendance.length : 0;
...
export const calcDecisions = (s) => {
  ...
  if (totalDebt > 0 && sipTotal > 0 && highRate) {
    const sipReturn = s.sips.length > 0
      ? s.sips.reduce((a, x) => a + x.returns, 0) / s.sips.length
      : 12;
    decisions.push(
      highRate.rate > sipReturn + 3
        ? { q: 'Invest or repay debt first?', rec: 'Repay Debt', reason: `${highRate.name} charges ${highRate.rate}% but SIPs return ~${sipReturn.toFixed(0)}%. Pay extra EMI — guaranteed ${highRate.rate}% return.`, icon: '🏦', color: '#F43F5E' }
        : { q: 'Invest or repay debt first?', rec: 'Keep Investing', reason: `SIP returns (~${sipReturn.toFixed(0)}%) exceed debt interest (${highRate.rate}%). Continue investing while paying minimum EMI.`, icon: '📈', color: '#22C55E' }
    );
  }

  decisions.push({
    q: 'Should I switch job now?',
    rec: s.salary < 80000 ? 'Switch if 35%+ hike' : 'Negotiate first',
    reason: s.salary < 80000
      ? `A 40% hike = ${inr(s.salary * 1.4)}/mo. At your level, switching > promoting.`
      : `At ${inr(s.salary)}/mo, negotiate 20-25% raise first. Switch only if denied.`,
    icon: '💼',
    color: '#3B82F6',
  });

  if (efSaved < efTarget * 0.5) {
    decisions.push({ q: 'Build emergency fund or invest?', rec: 'Emergency fund first', reason: `Only ${pct(efSaved, efTarget)}% of 6-month fund (${inr(efTarget)}) built. No emergency fund = one crisis can derail all goals.`, icon: '🛟', color: '#F59E0B' });
  }

  const lifeInsurance = s.salary * 12 * 10;
  decisions.push({ q: 'Do I have enough insurance?', rec: `Need ${inr(lifeInsurance)} life cover`, reason: `10x annual income rule: ${inr(lifeInsurance)} term cover. Pure term plan costs ₹800-1200/mo.`, icon: '🛡️', color: '#A78BFA' });
