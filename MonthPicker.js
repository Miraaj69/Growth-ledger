...
export default function MonthPicker({ month, year, onChange }) {
  ...
  return (
    <View style={styles.row}>
      <Pressable onPress={prev} style={styles.arrow}><Text style={styles.arrowText}>‹</Text></Pressable>
      <View style={styles.mid}>
        <Text style={styles.label}>{MONTHS_SHORT[month]} {year}</Text>
        {isCur && <Chip label="Now" color={Colors.green} dot />}
      </View>
      <Pressable onPress={next} style={[styles.arrow, isCur && styles.arrowDisabled]}><Text style={[styles.arrowText, isCur && { color: Colors.t4 }]}>›</Text></Pressable>
    </View>
  );
}
